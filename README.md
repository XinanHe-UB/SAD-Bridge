...
gradient-based AI-generated image detector.

- **Author:** Xinan He
- **Date:** 2026-07-10

SAD-Bridge detects AI-generated images without any classifier training.
It probes a MetaCLIP-2 vision encoder with a label-free self-consistency
...
systematically different geometry, which a k-NN classifier against a
small reference set can separate with high accuracy — generalizing
...
| Parameter      | Value |
|----------------|-------|
| Model weights  | [`facebook/metaclip-2-worldwide-giant`](https://huggingface.co/facebook/metaclip-2-worldwide-giant) |
| Architecture   | ViT-Giant vision encoder, MetaCLIP-2 "worldwide" release |
| Hidden size    | 1664 |
| Feature source | gradient of the vision encoder's intermediate hidden-state CLS token |

Weights are downloaded automatically from the HuggingFace Hub via
`transformers.AutoModel` / `AutoProcessor` on first run — no manual
...
| Feature layer          | a fixed intermediate vision-encoder layer (`TARGET_LAYER` in the scripts) |
| Classifier             | k-NN with cosine similarity against a labelled reference set |

This configuration was selected via a large-scale sweep over random
seeds, encoder layers, and k values, evaluated on a broad portfolio of
held-out AI-image-forensics benchmarks (GenImage, GenImage++,
Community-Forensics, AIGI_Holmes, DRCT, synthbuster, WildRF, CO-SPYBench,
corebench, NTIRE, RealChain, anime_test, and more).

## Files

| File | Purpose |
|------|---------|
| `extract_features.py` | Extracts the SAD-Bridge gradient feature for a folder of images. Used both to build a new **reference set** and to pre-extract **query features**. |
| `predict.py` | Runs k-NN classification of query images/features against a reference set. This is the detector's inference entry point. |
...
4. Compute the self-consistency loss: mean MSE between the CLS token of the
   anchor (0°) view and each rotated view, at the fixed feature layer.
5. Backpropagate; read the hidden-state gradient for that layer, shape `(4, seq, 1664)`.
...
The reference set is a feature cache with ground-truth labels, produced
by the same `extract_features.py` script. The published configuration
uses the GenImage `stable_diffusion_v_1_4` **train** split (`0_real` /
`1_fake` sub-folders) as the reference domain.
...
`predict.py` sub-samples and L2-normalises the reference set at load
time using its internal defaults — you only need to point `--ref-cache`
at the full extracted feature cache. To re-tune the sampling behaviour,
...
`predict.py`'s built-in default `--ref-cache` points at a pre-built
reference cache path from the original training run (see `_DEFAULT_REF_CACHE`
at the top of `predict.py`). That cache file is **not included** in this
repository (feature caches are large and not version-controlled here) —
you must either:

- build your own reference cache as described above and pass
  `--ref-cache reference.pt` explicitly, or
- copy a pre-built cache file into your own directory layout, or
...
| `--ref-cache` | see `_DEFAULT_REF_CACHE` in `predict.py`; not shipped in this repo — see §2 | Reference feature cache. |
...
| `fake_score` | Fraction of the nearest reference neighbours (k-NN) labelled *fake* (∈ [0, 1]). |
...
1. Load the reference cache, sub-sample it using the script's internal
   defaults, and L2-normalise.
2. Load (or extract) query features, L2-normalise.
3. Compute cosine similarity between every query and every reference vector.
4. Take the top-k most similar reference vectors; `fake_score` = fraction
   of those neighbours labelled fake.
...
├── predict.py             # official inference
├── requirements.txt       # Python dependencies
└── README.md              # this file
```

The scripts depend only on the reference feature cache described in
§2 above.
